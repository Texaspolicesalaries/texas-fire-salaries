...
(function () {
  ...
  // Resolve a distance origin from current-location coordinates or the in-memory
  // ZIP lookup cache. Department ZIPs remain a fast offline fallback.
  function resolveOrigin(st, departments) {
    if (st.nearLat && st.nearLng) return { lat: num(st.nearLat), lng: num(st.nearLng) };
    if (st.zip) {
      var zip = normalizeZip(st.zip);
      if (Object.prototype.hasOwnProperty.call(zipOrigins, zip)) return zipOrigins[zip];
      var local = window.TexasZipCentroids && window.TexasZipCentroids[zip];
      if (local) {
        zipOrigins[zip] = { lat: local[0], lng: local[1], zip: zip, place: local[2], state: 'Texas' };
        return zipOrigins[zip];
      }
      var d = departments.find(function (x) { return x.zip === zip; });
      if (d && typeof d.lat === 'number' && typeof d.lng === 'number') {
        zipOrigins[zip] = { lat: d.lat, lng: d.lng, zip: zip, place: d.city, state: 'Texas' };
        return zipOrigins[zip];
      }
    }
    return null;
  ...
  function resolveOriginAsync(st, departments) {
    return Promise.resolve(resolveOrigin(st, departments));
  }
  ...
  function makePredicate(st, ctx) {
    ...
    return function (d) {
      ...
      if (/^\d{5}$/.test(normalizeZip(st.zip)) && radius && !origin) return false;
      if (origin && radius) {
  ...
  window.FireFilters = {
    ...
    resolveOriginAsync: resolveOriginAsync,
